...
const char *WINDOWTITLE = { "Assignment 6: Nathan Shepherd" };
...
Display( )
{
	...
	for (size_t i = 0; i < 8; i++) {
		Curve c = {
			.2, .2, 1., //Color .//18*cos(4.*M_PI*Time), 1.8, .18*sin(4.*M_PI*Time
			last_point, // p0
			{ offset-.4, 0., last_point.z}, // p1
			{ offset+.6, 0., .4*cos(M_PI*timestand*i)}, // p2
			{ offset+1.+ .2*sin(2.*M_PI*Time), height + .2*cos(2.*M_PI*Time), .4*cos(M_PI*timestand*i)}  // p3
		};

		DrawCurve(c);
		offset++;
		last_point.x++;
		last_point.z = .4*cos(M_PI*timestand*i);
