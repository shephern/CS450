...
const char *WINDOWTITLE = { "Assignment 2: Nathan Shepherd" };
const char *GLUITITLE   = { "Window Interface" };
...
int 	WhichPosition;      // INSIDE or OUTSIDE
int		Xmouse, Ymouse;			// mouse values
float	Xrot, Yrot;				// rotation angles in degrees

float BladeSpeed = 5000;
bool Frozen = false;
...
Display( )
{
	...
	if( WhichPosition == OUTSIDE )
		gluLookAt( 12., 1., 4.,     0., 0., 0.5,     0., 1., 0. );
	else
		gluLookAt(-0.4, 1.4, -4.1,     0., 1.5, -9.,     0., 1., 0. );
	...
	glRotatef(Time * BladeSpeed, 0., 1., 0.);
	glScalef( 5., 1., 1. );
	glRotatef( 90., 1., 0., 0. );
	glCallList( propsList );
	glPopMatrix();

	glPushMatrix();
	glTranslatef( .5, 1.3, 6.3 );
	glRotatef(Time * BladeSpeed * 3, 1., 0., 0.);
	glScalef( 1., 1., 1.5 );
	glRotatef(90., 0., 1., 0. );
	glCallList( propsList );
	glPopMatrix();
...
InitLists( )
{
	...
	glNewList( RockList, GL_COMPILE );
		glPushMatrix( );
		glTranslatef(0., offset, z_offset);
		glScalef(3.,3.,3.);
	...
	propsList = glGenLists( 1 );
	glNewList( propsList, GL_COMPILE );
		glColor3f(1., 1., 1.);
		glBegin( GL_TRIANGLES );
