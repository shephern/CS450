...
GLuint earth, jupiter, mars, venus;

unsigned char *Texture;
unsigned char *earthTex;
unsigned char *jupiterTex;
unsigned char *marsTex;
unsigned char *venusTex;
...
Display( )
{
	...
	width = 1024;
	height = 512;
	earthTex = BmpToTexture( "textures/earth.bmp", &width, &height );
	jupiterTex = BmpToTexture( "textures/jupiter.bmp", &width, &height );
	marsTex = BmpToTexture( "textures/mars.bmp", &width, &height );
 	venusTex = BmpToTexture( "textures/venus.bmp", &width, &height );
	...
	switch (WhichTexture) {
		case EARTH:
			...
			glTexImage2D( GL_TEXTURE_2D, level, ncomps, width, height, border, GL_RGB, GL_UNSIGNED_BYTE, earthTex );
			break;
		case JUPITER:
			glBindTexture( GL_TEXTURE_2D, jupiter);
			glTexImage2D( GL_TEXTURE_2D, level, ncomps, width, height, border, GL_RGB, GL_UNSIGNED_BYTE, jupiterTex );
			break;
		case MARS:
			glBindTexture( GL_TEXTURE_2D, mars);
			glTexImage2D( GL_TEXTURE_2D, level, ncomps, width, height, border, GL_RGB, GL_UNSIGNED_BYTE, marsTex );
			break;
		case VENUS:
			glBindTexture( GL_TEXTURE_2D, venus);
			glTexImage2D( GL_TEXTURE_2D, level, ncomps, width, height, border, GL_RGB, GL_UNSIGNED_BYTE, venusTex );
			break;
		default:
			glBindTexture( GL_TEXTURE_2D, venus);
			glTexImage2D( GL_TEXTURE_2D, level, ncomps, width, height, border, GL_RGB, GL_UNSIGNED_BYTE, venusTex );
...
InitMenus( )
{
	...
	glutAddMenuEntry( "Distortion", DISTORT );
...
MjbSphere( float radius, int slices, int stacks )
{
	...
	for( int ilat = 0; ilat < NumLats; ilat++ )
	{
		...
		for( int ilng = 0; ilng < NumLngs; ilng++ )
		{
			...
			if( WhichMode == DISTORT )
			{
				p->s = (lng + M_PI ) / ( 2.*M_PI ) + y * .3 * timestand;
				p->t = (.02*x*timestand+lat + M_PI/2. ) / M_PI;
			}
			else
			{
				p->s = ( lng + M_PI    ) / ( 2.*M_PI );
